...
var strat = {
	...
	init: function () {
		// core
		this.name = 'EMA Cross Trailing Only Close';
	...
	check: function (candle) {
		...
		const long_cond = price > emaFast && price > emaSlow && !(prev_price > prev_emaFast && prev_price > prev_emaSlow)
		const short_cond = price < emaFast && price < emaSlow
		...
		this.stop_price = this.max_run_up * (1 - this.settings.TRAILING / 100);
