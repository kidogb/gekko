...
var strat = {
  ...
  init: function () {
    // core
    this.name = 'RSI Bull and Bear + ADX M1 + Stop Loss + Short';
    ...
    // check in position or not (in case without stop/loss)
    this.inPosition = false;    // actual position
    this.inFakePosition = false;  // position if dont have stop/loss
    this.entry = undefined;
    this.exit = undefined;

    // for performance report
    this.drawDawn = undefined;
    this.runUp = undefined;
  ...
  check: function (candle) {
    ...
    // CALCULATE STOPLOSS
    if (this.inPosition) {
      const stopPrice = (1 + this.settings.STOP/100) * this.entry;
      // const takeProfit = (1 + this.settings.TAKE_PROFIT/100) * this.entry;
    ...
    if (maFast < maSlow) {
      ...
      if (!this.inFakePosition && rsi > rsi_hi) {
        this.long(candle);
        this.inFakePosition = true;
      }
      else if (this.inFakePosition && rsi < rsi_low) {
        this.short(candle);
        this.inFakePosition = false;
      }
